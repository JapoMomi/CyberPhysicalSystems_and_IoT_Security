...
This system implements a secure mutual authentication protocol between IoT devices and a central server,
using AES-128 encryption and dynamic key management through a secure vault mechanism. 

Additional ECC authentication program is executed at completion for comparative analysis.
"""
...
CHALLENGE_2_LENGTH = 3  # Number of keys to combine for second challenge

class SecureVault:
    """
    Secure cryptographic key storage and management system
    Implements dynamic key updates using HMAC-based key derivation
    
    Attributes:
        keys (list): Array of cryptographic keys stored as bytes
    """
...
class IoTServer(threading.Thread):
    """
    Central authentication server handling multiple IoT devices
    Manages secure sessions and cryptographic challenges
    
    Attributes:
        device_vaults (dict): Mapping of device IDs to their SecureVault instances
        sessions (dict): Active authentication sessions
...
class IoTDevice(threading.Thread):
    """
    IoT device implementation handling secure authentication
    
    Attributes:
        device_id (str): Unique device identifier
        server (IoTServer): Reference to central server
        vault (SecureVault): Device-specific key vault
        authenticated (bool): Authentication status
        encrypt_time (float): Challenge response encryption time
        decrypt_time (float): Final response decryption time
        vault_update_time (float): Vault update duration
    """
